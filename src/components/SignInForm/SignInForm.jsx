  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        setMessage({ type: 'error', text: error || 'Login failed. Please try again.' });
      } finally {
        setLoading(false);
      }
    },
    ...
    <div className={css.formContainer}>
      <form className={css.form} onSubmit={formik.handleSubmit}>
          ...
          <div className={css.passwordWrapper}>
            ...
            >
              <svg className={css.icon}>
                <use href={`${sprite}#icon-${showPassword ? 'eye-off' : 'eye'}`} />
              </svg>
            </button>
          </div>
        ...
        <p className={css.signupLink}>
          Don’t have an account? <Link to="/signup" className={css.link}>Sign Up</Link>
        </p>
      </form>
    </div>
  );
